...
$.get("../data/products.csv", function (data) {
  ...
  //Load Hot Products
  loadHotProducts();
  /*****************   Loading Master Data Section End  **************/
...
function populateProducts(page) {
  ...
  if (products.length > 0) {
    ...
    if (page == undefined) {
      ...
      startAt = (parseInt(page) - 1) * 6;

      if (products.length - startAt < 6) {
        endAt = products.length;
...
function populateFilters() {
  ...
  for (var c in categoryList) {
    ...
    $("#" + categoryList[c].name + "").prop(
      "checked",
      categoryList[c].filter
    );
  }
  ...
  if (products.length > 0) {
    ...
    if (products.length <= 6) {
      ...
      for (var i = 2; i <= pages; i++) {
        $("#pagination").append('  <li><a href="#" class="nextPage">' + i + "</a></li>");
      }
    }
  }
}
...
function bindWhatspp() {
  $(".sendMessage").on("click", function () {
    ...
    var url =
      ...
      "&text=%20" +
      message +
      "%0a Click here to see the product image (http://127.0.0.1:5500/img/" +
      product.name +
      ".png)";
